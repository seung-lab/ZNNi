...
public:
    ...
    explicit deshuffler( vec3i const & s )
        ...
        , len(s[0]*s[1]*s[2])
    {
        mappings = get_array<uint32_t>(len);
        for ( long_t i = 0; i < len; ++i )
        {
            mappings.get()[i] = static_cast<uint32_t>(i);
        }
    }
    ...
    void split( vec3i const & s )
    {
        ...
        auto new_mappings = get_array<uint32_t>(len);

        long_t delta = current_size[0] * current_size[1] * current_size[2];

        for ( long_t i = 0; i < n; ++i )
        {
            single_shuffle( mappings.get() + i * delta,
                            new_mappings.get() + i * delta,
                            current_size,
                            s );
        }
    ...
    host_array<real> shuffle(host_array<real> in) const
    {
        ...
        for ( long_t i = 0; i < len; ++i )
        {
            ret.get()[i] = in.get()[mappings.get()[i]];
        }
    ...
    host_array<real> deshuffle(host_array<real> in) const
    {
        ...
        for ( long_t i = 0; i < len; ++i )
        {
            ret.get()[mappings.get()[i]] = in.get()[i];
        }
