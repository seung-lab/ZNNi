...
void test_conv_layer()
{
    ...
    long_t n = intdist(rng.rng);
    long_t l = intdist2(rng.rng);
    long_t l2 = 1;//intdist2(rng.rng);
    ...
    cpu::padded_pruned_fft_convolutional_layer
        np(n,l,l2,is,ws, kernels.get(), biases.get());

    znn::fwd::tbb::padded_pruned_fft_auto_convolutional_layer
        pl(n,l,l2,is,ws, kernels.get(), biases.get());
