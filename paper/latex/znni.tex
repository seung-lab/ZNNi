...
\subsection{Advanced network execution benchmarks}
  ...
  layer, where data parallel algorithm is used.

  \subsubsection{GPU + host RAM}

  First 4 layers are done layer at the time using GPU + host RAM
  implementation.  As there are 2 pooling layers in the first 4
  layers, the output of the first four layers has a mini--batch size
  of 64.  The latter layers are applied mini--batch at the time, where
  the size of minibatch is maximized so that it can fit on the GPU.
