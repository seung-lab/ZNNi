...
when large numbers of images and/or very large images are to be
processed.  Inference is especially costly when a ConvNet is applied
in a window that slides over every location in a large image.  Sliding
window ConvNets have been used successfully for image segmentation
(cite), object detection and localization (cite), and are generally
useful for image to image transformations.
...
\section{Pruned FFT}
  The efficiency of convolution is important, because this computation
  is performed so many times in a ConvNet.  The advantages of FFT
  convolution have been shown for 2D ConvNets running on
  ...
  understanding the rest of our contributions.
...
%% we open a new space of exploration as the current ones depend only on the GPU computational power.  given the new efficent algorithms for different architectures, we make some instersting obsrvations... which are...
%% we discuss why they are what we exepct them to be
