...
\section{Introduction}
  ...
  image borders turns out to matter. Because inference is less
  efficient at locations near image borders, it is advantageous to
  make the input image large so that border effects are proportionally
  smaller.  This means that efforts to maximize throughput tend to
  bump into the constraint of limited RAM.  (This is not the case for
  ConvNet training, where there is typically some range of input sizes
  that is optimal for training speed.)  It follows that an apparently
...
\subsection{CPU--GPU ConvNet execution}
  ...
  stored in the host ram, along and memory allocated for the network
