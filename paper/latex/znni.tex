...
time.  Other things being equal, processing larger image sizes tends
to increase throughput, because fractionally less computation is
wasted on the borders of the image.  It follows that an apparently
slower algorithm may end up having higher throughput if it can process
a larger image within the constraint of the available RAM.  Therefore
we propose a number of CPU and GPU primitives for convolutional and
pooling layers, which are designed to minimize memory overhead.  The
primitives are combined to create CPU-only, GPU-only, GPU + host RAM,
and CPU-GPU inference algorithms.  The primitives include convolution
based on highly efficient pruned FFTs. Our theoretical analyses and
empirical tests reveal a number of interesting findings.  First,
throughput is typically maximized for the CPU-only and GPU-only
algorithms by choosing different primitives for successive layers.
Second, it may be advantageous for the GPU to use host RAM for storage
of intermediate results, though onboard RAM has much faster access.
Third, our CPU-GPU algorithm can achieve greater throughput than the
sum of CPU-only and GPU-only throughputs.  Fourth, adding host RAM can
be a more efficient way of increasing throughput than adding another
GPU or more CPUs.
\end{abstract}
