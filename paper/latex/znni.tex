...
\section{Network execution}
  ...
  For a given problem the minimization problem can be solved using
  exhaustive search over the output shapes and layer implementations
  and collecting empirical results, which can be computationally very
  intensive.  As it turns out, with theoretical analysis of different
  implementations' computational complexities and memory requirements,
  we can narrow the search space.

\subsection{Max--pooling vs max--fragment--pooling}

  Giusti at al.~\cite{giusti2013fast} and Masci at
  al.~\cite{masci2013fast} show that \emph{max--fragment--pooling}
  ...
  \emph{max--fragment--pooling} layer $i$ with a window size of
  ...
  \begin{figure}
    ...
    \caption{Comparison of the theoretical speedups of max--pooling
      and max--fragment--pooling ConvNets.}
