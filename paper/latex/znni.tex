...
\section{FFT of padded image}
  ...
  kernel, it would become much faster.  We propose such an algorithm,
  and implement it for both the CPU and GPU. Our approach achieves an
  average of $5 \times$ speedup over the naive approach on the CPU and
  $10 \times$ speedup on the GPU.  It also greatly reduces the memory
  overhead compared to the approaches described in
  ~\cite{mathieu-iclr-14,vasilache2014fast}.
...
\subsubsection{Direct convolution algorithm}
  ...
  The $\kw{parallel for}$ loops are implemented using intel$^{\footnotesize
  ...
  using Intel$^{\footnotesize \textregistered}$ to perform convolutions.  MKL
...
\subsection{GPU implementations}
  ...
  matrix--matrix multiplication~\cite{chetlur2014cudnn}.  The third
  FFT--based implementation is based on our, previously described,
  algorithm for padded--pruned FFTs.
