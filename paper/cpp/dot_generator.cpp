...
void dump_task_dependencies(int fin, int fout, int bs)
{
    ...
    // Forward FFT images
    for ( int x = 0; x < bs * fin; ++x )
    {
        dump_node("fwd_in_image",x,red);
        dump_edge("sp1","fwd_in_image",x);
        dump_edge("fwd_in_image",x,"sp2");
    }
    ...
    for ( int x = 0; x < fout; ++x )
    {
        for ( int y = 0; y < fin; ++y )
        {
            ...
            for ( int z = 0; z < bs; ++z )
            {
                ...
                if ( y == (fin-1) ) dump_edge("mad",x,y,z,"sp",3);
    ...
    // Forward FFT images
    for ( int x = 0; x < bs * fout; ++x )
    {
        dump_node("bwd_out_image",x,purple);
        dump_edge("sp3","bwd_out_image",x);
        dump_edge("bwd_out_image",x,"sp4");
    }
    of << "}\n";
}
...
int main()
{
    ...
    of.open("deps.DOT");
    dump_task_dependencies(10,10,8);
    of.close();
}
