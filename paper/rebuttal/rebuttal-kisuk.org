...
- Novel parallel primitives for both the CPU and GPU that use FFT
  based convolution.
...
2) Relationship to Caffe con Trolls
   ...
   possible.  For instance, on our 72 core machine, CcT approach would
   limit the memory per input to 256/72 = 3.5 GB, which would yield
   significantly smaller throughput (10x or more).
...
- TODO(lee): We might need to mention that the choice of depth and width of the
  benchmark networks was restricted by the fact that deeper and wider 3D
  networks tend to not even fit in the GPU memory when using Caffe or ELEKTRONN,
  making it impossible to compare our proposed method against them. We tried to
  make the benchmark networks as deep and wide as possible under the constraint
  that the networks should at least be able to fit in the GPU memory
  when using Caffe and ELEKTRONN.
...
- In our task-based primitive, the mention the perormance gain of our
  problem-specific task scheduling compared to standard scheduling
