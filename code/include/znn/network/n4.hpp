...
create_n4(const vec3i outsz)
{
  ...
  float conv1_b[48];
  read_from_file<float>("./zfish-N4_A/conv1/kernels",conv1_k,1*48*4*4);
  ...
  float conv2_b[48];
  read_from_file<float>("./zfish-N4_A/conv2/filters",conv2_k,48*48*5*5);
  ...
  // conv3
  float conv3_k[48*48*4*4];
  float conv3_b[48];
  read_from_file<float>("./zfish-N4_A/conv3/filters",conv3_k,48*48*4*4);
  read_from_file<float>("./zfish-N4_A/nconv3/biases",conv3_b,48);
  layers.push_back(std::unique_ptr<host::v1::host_layer>
                    (new host::v1::direct_conv
                     (16, 48, 48,
                      vec3i(1,21,21)+outsz-vec3i(1,1,1), vec3i(1,4,4)+outsz-vec3i(1,1,1),
                      conv3_k, conv3_b)));
  ...
  // conv4
  float conv4_k[48*48*4*4];
  float conv4_b[48];
  read_from_file<float>("./zfish-N4_A/conv4/filters",conv4_k,48*48*4*4);
  read_from_file<float>("./zfish-N4_A/nconv4/biases",conv4_b,48);
  layers.push_back(std::unique_ptr<host::v1::host_layer>
                    (new host::v1::direct_conv
                     (64, 48, 48,
                      vec3i(1,9,9)+outsz-vec3i(1,1,1), vec3i(1,6,6)+outsz-vec3i(1,1,1),
                      conv4_k, conv4_b)));
  ...
  // conv5
  float conv5_k[48*48*3*3];
  float conv5_b[48];
  read_from_file<float>("./zfish-N4_A/conv5/filters",conv5_k,48*48*3*3);
  read_from_file<float>("./zfish-N4_A/nconv5/biases",conv5_b,48);
  layers.push_back(std::unique_ptr<host::v1::host_layer>
                    (new host::v1::direct_conv
                     (256, 48, 200,
                      vec3i(1,3,3)+outsz-vec3i(1,1,1), vec3i(1,1,1)+outsz-vec3i(1,1,1),
                      conv5_k, conv5_b)));
  ...
  read_from_file<float>("./zfish-N4_A/nconv6/biases",conv6_b,200);
  layers.push_back(std::unique_ptr<host::v1::host_layer>
                    (new host::v1::direct_conv
                     (256, 200, 3,
                      vec3i(1,1,1)+outsz-vec3i(1,1,1), vec3i(1,1,1)+outsz-vec3i(1,1,1),
                      conv6_k, conv6_b)));
