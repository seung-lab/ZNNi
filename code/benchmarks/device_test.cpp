...
#include "znn/device/2dv2/softmax.hpp"
...
void xxxxx()
{
    ...
    layers.push_back(make_unique<device::twod::conv>(1,3,64,vec2i(1028,1028),vec2i(4,4),k,b));
    layers.push_back(make_unique<device::twod::maxout>(1,64,2,vec2i(1025,1025)));
    layers.push_back(make_unique<device::twod::mfp>(1,32,vec2i(1025,1025),vec2i(2,2)));

    read_from_file<float>("./exper_aleks/layer2.kernels",k,32*64*4*4);
    read_from_file<float>("./exper_aleks/layer2.biases",b,64);
    layers.push_back(make_unique<device::twod::conv>(4,32,64,vec2i(512,512),vec2i(4,4),k,b));
    layers.push_back(make_unique<device::twod::mfp>(4,64,vec2i(509,509),vec2i(2,2)));
    layers.push_back(make_unique<device::twod::maxout>(16,64,2,vec2i(254,254)));

    read_from_file<float>("./exper_aleks/layer3.kernels",k,32*64*4*4);
    read_from_file<float>("./exper_aleks/layer3.biases",b,64);
    layers.push_back(make_unique<device::twod::conv>(16,32,64,vec2i(254,254),vec2i(4,4),k,b));
    // layers.push_back(make_unique<device::twod::mfp>(16,64,vec2i(251,251),vec2i(2,2)));
    // layers.push_back(make_unique<device::twod::maxout>(64,64,2,vec2i(125,125)));

    layers.push_back(make_unique<device::twod::maxout>(16,64,2,vec2i(251,251)));
    layers.push_back(make_unique<device::twod::mfp>(16,32,vec2i(251,251),vec2i(2,2)));


    read_from_file<float>("./exper_aleks/layer4.kernels",k,32*4*4*4);
    read_from_file<float>("./exper_aleks/layer4.biases",b,4);
    layers.push_back(make_unique<device::twod::conv>(64,32,4,vec2i(125,125),vec2i(4,4),k,b));

    layers.push_back(make_unique<device::twod::softmax>(64,4,vec2i(122,122)));
    ...
    for ( long_t stages = 0; stages < 254; ++stages )
    {
        ...
        //device_tensor_ref<float,4> dresult(inout[lnum%2].data(), 64, 4, 122, 122);
        hresultx.load(inout[lnum%2].data(), from_device);
